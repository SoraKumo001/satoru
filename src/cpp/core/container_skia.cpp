...
litehtml::uint_ptr container_skia::create_font(const litehtml::font_description &desc,
    ...
    for(auto& tf : m_context.fallbackTypefaces) {
        ...
        for(auto& existing : typefaces) {
            if(existing.get() == tf.get()) { found = true; break; }
        }
    ...
    if (fm) {
        fm->ascent = (int)-skfm.fAscent; fm->descent = (int)skfm.fDescent;
        fm->height = fm->ascent + fm->descent; fm->x_height = (int)skfm.fXHeight;
    }
    fi->fm_ascent = (int)-skfm.fAscent; fi->fm_height = fi->fm_ascent + (int)skfm.fDescent;
    return (litehtml::uint_ptr)fi;
}

void container_skia::delete_font(litehtml::uint_ptr hFont) {
    font_info *fi = (font_info *)hFont;
    if (fi) { for (auto font : fi->fonts) delete font; delete fi; }
}
...
litehtml::pixel_t container_skia::text_width(const char *text, litehtml::uint_ptr hFont) {
    ...
    while (*p) {
        ...
        for (auto f : fi->fonts) {
            SkGlyphID glyph; f->getTypeface()->unicharsToGlyphs(SkSpan<const SkUnichar>((const SkUnichar *)&u, 1), SkSpan<SkGlyphID>(&glyph, 1));
            if (glyph != 0) { font = f; break; }
        }
        if (!font) font = fi->fonts[0];
        if (font != current_font) {
            if (current_font && p > run_start) total_width += current_font->measureText(run_start, p - run_start, SkTextEncoding::kUTF8);
            run_start = p; current_font = font;
        }
        p = next_p;
    }
    if (current_font && p > run_start) total_width += current_font->measureText(run_start, p - run_start, SkTextEncoding::kUTF8);
...
void container_skia::draw_text(litehtml::uint_ptr hdc, const char *text, litehtml::uint_ptr hFont,
    ...
    SkPaint paint; paint.setColor(SkColorSetARGB(color.alpha, color.red, color.green, color.blue)); paint.setAntiAlias(true);
    double x_offset = 0; const char *p = text; const char *run_start = p; SkFont *current_font = nullptr;
    while (*p) {
        const char *next_p = p; char32_t u = decode_utf8(&next_p); SkFont *font = nullptr;
        for (auto f : fi->fonts) {
            SkGlyphID glyph; f->getTypeface()->unicharsToGlyphs(SkSpan<const SkUnichar>((const SkUnichar *)&u, 1), SkSpan<SkGlyphID>(&glyph, 1));
        ...
        if (font != current_font) {
            if (current_font && p > run_start) {
                SkFont render_font = *current_font; if (fi->fake_bold) render_font.setEmbolden(true);
                m_canvas->drawSimpleText(run_start, p - run_start, SkTextEncoding::kUTF8, (float)pos.x + (float)x_offset, (float)pos.y + (float)fi->fm_ascent, render_font, paint);
...
void container_skia::draw_box_shadow(litehtml::uint_ptr hdc, const litehtml::shadow_vector &shadows, const litehtml::position &pos, const litehtml::border_radiuses &radius, bool inset) {
    ...
    for (const auto &s : shadows) {
        if (s.inset != inset) continue;
        SkRRect box_rrect = make_rrect(pos, radius); SkColor shadow_color = SkColorSetARGB(s.color.alpha, s.color.red, s.color.green, s.color.blue); float blur_std_dev = (float)s.blur.val() * 0.5f;
        m_canvas->save();
        if (inset) {
            m_canvas->clipRRect(box_rrect, true); SkRRect shadow_rrect = box_rrect; shadow_rrect.inset(-(float)s.spread.val(), -(float)s.spread.val());
            SkPaint p; p.setAntiAlias(true); p.setColor(shadow_color); if (blur_std_dev > 0) p.setMaskFilter(SkMaskFilter::MakeBlur(kNormal_SkBlurStyle, blur_std_dev));
            SkRect hr = box_rrect.rect(); hr.outset(blur_std_dev * 3 + std::abs((float)s.x.val()) + 100, blur_std_dev * 3 + std::abs((float)s.y.val()) + 100);
            m_canvas->translate((float)s.x.val(), (float)s.y.val()); m_canvas->drawPath(SkPathBuilder().addRect(hr).addRRect(shadow_rrect, SkPathDirection::kCCW).detach(), p);
        } else {
            m_canvas->clipRRect(box_rrect, SkClipOp::kDifference, true); SkRRect shadow_rrect = box_rrect; shadow_rrect.outset((float)s.spread.val(), (float)s.spread.val());
            SkPaint p; p.setAntiAlias(true); p.setColor(shadow_color); if (blur_std_dev > 0) p.setMaskFilter(SkMaskFilter::MakeBlur(kNormal_SkBlurStyle, blur_std_dev));
            m_canvas->translate((float)s.x.val(), (float)s.y.val()); m_canvas->drawRRect(shadow_rrect, p);
        }
        m_canvas->restore();
    }
}
...
void container_skia::draw_image(litehtml::uint_ptr hdc, const litehtml::background_layer &layer, const std::string &url, const std::string &base_url) {
    ...
    if (m_tagging) {
        ...
        if (it != m_context.imageCache.end() && it->second.skImage) {
            SkPaint p; p.setAntiAlias(true); m_canvas->save(); m_canvas->clipRRect(make_rrect(layer.border_box, layer.border_radius), true);
            m_canvas->drawImageRect(it->second.skImage, SkRect::MakeXYWH((float)layer.origin_box.x, (float)layer.origin_box.y, (float)layer.origin_box.width, (float)layer.origin_box.height), SkSamplingOptions(SkFilterMode::kLinear), &p);
            m_canvas->restore();
        }
    }
}
...
void container_skia::draw_solid_fill(litehtml::uint_ptr hdc, const litehtml::background_layer &layer, const litehtml::web_color &color) {
    if (!m_canvas) return;
    SkPaint p; p.setColor(SkColorSetARGB(color.alpha, color.red, color.green, color.blue)); p.setAntiAlias(true); m_canvas->drawRRect(make_rrect(layer.border_box, layer.border_radius), p);
}
...
void container_skia::draw_linear_gradient(litehtml::uint_ptr hdc, const litehtml::background_layer &layer, const litehtml::background_layer::linear_gradient &gradient) {
    if (!m_canvas) return;
    SkPoint pts[2] = {SkPoint::Make((float)gradient.start.x, (float)gradient.start.y), SkPoint::Make((float)gradient.end.x, (float)gradient.end.y)};
    ...
    SkGradient grad(SkGradient::Colors(SkSpan(colors), SkSpan(pos), SkTileMode::kClamp), SkGradient::Interpolation());
    SkPaint p; p.setShader(SkShaders::LinearGradient(pts, grad)); p.setAntiAlias(true); m_canvas->drawRRect(make_rrect(layer.border_box, layer.border_radius), p);
}
...
void container_skia::draw_radial_gradient(litehtml::uint_ptr hdc, const litehtml::background_layer &layer, const litehtml::background_layer::radial_gradient &gradient) {
    if (!m_canvas) return;
    SkPoint center = SkPoint::Make((float)gradient.position.x, (float)gradient.position.y); float rx = (float)gradient.radius.x, ry = (float)gradient.radius.y; if (rx <= 0 || ry <= 0) return;
    std::vector<SkColor4f> colors; std::vector<float> pos;
    for (const auto &stop : gradient.color_points) {
        colors.push_back({stop.color.red / 255.0f, stop.color.green / 255.0f, stop.color.blue / 255.0f, stop.color.alpha / 255.0f});
        pos.push_back(stop.offset);
    ...
    SkGradient grad(SkGradient::Colors(SkSpan(colors), SkSpan(pos), SkTileMode::kClamp), SkGradient::Interpolation());
    SkPaint p; p.setShader(SkShaders::RadialGradient(center, rx, grad, &matrix)); p.setAntiAlias(true); m_canvas->drawRRect(make_rrect(layer.border_box, layer.border_radius), p);
}
...
void container_skia::draw_conic_gradient(litehtml::uint_ptr hdc, const litehtml::background_layer &layer, const litehtml::background_layer::conic_gradient &gradient) {
    ...
    if (m_tagging) {
        ...
        SkGradient grad(SkGradient::Colors(SkSpan(colors), SkSpan(pos), SkTileMode::kClamp), SkGradient::Interpolation());
        SkPaint p; p.setShader(SkShaders::SweepGradient(center, gradient.angle, gradient.angle + 360.0f, grad)); p.setAntiAlias(true); m_canvas->drawRRect(make_rrect(layer.border_box, layer.border_radius), p);
    }
}
...
void container_skia::draw_borders(litehtml::uint_ptr hdc, const litehtml::borders &borders, const litehtml::position &draw_pos, bool root) {
    if (!m_canvas) return;
    bool uniform = borders.top.width == borders.bottom.width && borders.top.width == borders.left.width && borders.top.width == borders.right.width && borders.top.color == borders.bottom.color && borders.top.color == borders.left.color && borders.top.color == borders.right.color;
    if (uniform && borders.top.width > 0) {
        SkPaint p; p.setColor(SkColorSetARGB(borders.top.color.alpha, borders.top.color.red, borders.top.color.green, borders.top.color.blue)); p.setStrokeWidth((float)borders.top.width); p.setStyle(SkPaint::kStroke_Style); p.setAntiAlias(true);
        SkRRect rr = make_rrect(draw_pos, borders.radius); rr.inset((float)borders.top.width / 2.0f, (float)borders.top.width / 2.0f); m_canvas->drawRRect(rr, p);
    } else {
        litehtml::web_color common = {0,0,0,0}; bool mc = false; int active = 0;
        auto chk = [&](const litehtml::border &b) { if (b.width > 0 && b.style != litehtml::border_style_none && b.style != litehtml::border_style_hidden) { if (active == 0) common = b.color; else if (b.color != common) mc = true; active++; } };
        chk(borders.top); chk(borders.bottom); chk(borders.left); chk(borders.right);
        if (active == 0) return;
        m_canvas->save(); SkRRect outer = make_rrect(draw_pos, borders.radius);
        if (!mc) {
            ...
            auto dseg = [&](const litehtml::border &b, float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4) { if (b.width <= 0 || b.style == litehtml::border_style_none || b.style == litehtml::border_style_hidden) return; SkPaint p; p.setColor(SkColorSetARGB(b.color.alpha, b.color.red, b.color.green, b.color.blue)); p.setAntiAlias(true); m_canvas->drawPath(SkPathBuilder().moveTo(x1,y1).lineTo(x2,y2).lineTo(x3,y3).lineTo(x4,y4).close().detach(), p); };
            float x = (float)draw_pos.x, y = (float)draw_pos.y, w = (float)draw_pos.width, h = (float)draw_pos.height, lw = (float)borders.left.width, tw = (float)borders.top.width, rw = (float)borders.right.width, bw = (float)borders.bottom.width;
            dseg(borders.top, x,y, x+w,y, x+w-rw,y+tw, x+lw,y+tw); dseg(borders.bottom, x,y+h, x+w,y+h, x+w-rw,y+h-bw, x+lw,y+h-bw); dseg(borders.left, x,y, x+lw,y+tw, x+lw,y+h-bw, x,y+h); dseg(borders.right, x+w,y, x+w-rw,y+tw, x+w-rw,y+h-bw, x+w,y+h);
        }
        m_canvas->restore();
    }
}
...
void container_skia::load_image(const char *src, const char *baseurl, bool redraw_on_ready) { if (m_resourceManager && src && *src) m_resourceManager->request(src, src, ResourceType::Image); }
void container_skia::get_image_size(const char *src, const char *baseurl, litehtml::size &sz) { int w, h; if (m_context.get_image_size(src, w, h)) { sz.width = w; sz.height = h; } else { sz.width = 0; sz.height = 0; } }
void container_skia::get_viewport(litehtml::position &viewport) const { viewport.x = 0; viewport.y = 0; viewport.width = m_width; viewport.height = m_height; }
void container_skia::transform_text(litehtml::string &text, litehtml::text_transform tt) { if (text.empty()) return; if (tt == litehtml::text_transform_uppercase) for (auto &c : text) c = (char)toupper(c); else if (tt == litehtml::text_transform_lowercase) for (auto &c : text) c = (char)tolower(c); }
void container_skia::import_css(litehtml::string &text, const litehtml::string &url, litehtml::string &baseurl) { if (!url.empty() && m_resourceManager) m_resourceManager->request(url, url, ResourceType::Css); else scan_font_faces(text); }
void container_skia::scan_font_faces(const std::string &css) {
    std::regex fontFaceRegex("@font-face\\s*\\{([^}]+)\\}", std::regex::icase); std::regex familyRegex("font-family:\\s*([^;]+);?", std::regex::icase); std::regex weightRegex("font-weight:\\s*([^;]+);?", std::regex::icase); std::regex styleRegex("font-style:\\s*([^;]+);?", std::regex::icase); std::regex urlRegex("url\\s*\\(\\s*['\"]?([^'\")]+)['\"]?\\s*\\)", std::regex::icase);
    ...
    for (std::sregex_iterator i = words_begin; i != std::sregex_iterator(); ++i) {
        std::string body = (*i)[1].str(); std::smatch m; font_request req; req.weight = 400; req.slant = SkFontStyle::kUpright_Slant;
...
std::string container_skia::get_font_url(const std::string &family, int weight, SkFontStyle::Slant slant) const {  
    ...
    while (std::getline(ss, item, ',')) {
        ...
        int minDiff = 1000; std::string bestUrl = ""; for (const auto &pair : m_fontFaces) { if (pair.first.family == cleanFamily && pair.first.slant == slant) { int diff = std::abs(pair.first.weight - weight); if (diff < minDiff) { minDiff = diff; bestUrl = pair.second; } } }
        if (!bestUrl.empty()) return bestUrl;
    }
    return "";
}
void container_skia::set_clip(const litehtml::position &pos, const litehtml::border_radiuses &bdr_radius) { if (m_canvas) m_canvas->save(), m_canvas->clipPath(SkPathBuilder().addRRect(make_rrect(pos, bdr_radius)).moveTo(0, 0).lineTo(0, 0).detach(), true); m_last_clip_pos = pos; m_last_clip_radius = bdr_radius; }
void container_skia::del_clip() { if (m_canvas) m_canvas->restore(); }
void container_skia::get_media_features(litehtml::media_features &features) const { features.type = litehtml::media_type_screen; features.width = m_width; features.height = m_height; features.device_width = m_width; features.device_height = m_height; features.color = 8; features.monochrome = 0; features.color_index = 256; features.resolution = 96; }
void container_skia::get_language(litehtml::string &language, litehtml::string &culture) const { language = "en"; culture = "en-US"; }